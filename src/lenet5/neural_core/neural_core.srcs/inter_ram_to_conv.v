...
(
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin 
			...
			row1_valid <= row0_valid;
        	row2_valid <= row1_valid;
        	row3_valid <= row2_valid;
        end
        else begin
			row1_valid <= row1_valid;
        	row2_valid <= row2_valid;
        	row3_valid <= row3_valid;
        end
    end
    ...
    always @(posedge i_clk) begin
        ...
        else if (rows_valid & ~output_done) output_counter <= output_counter + 'd1;
        else output_counter = output_counter;
    end

    assign output_done = (output_counter == (row_width - kernel_width + 1));

    // rows_valid logic
    assign rows_valid = row1_valid & row2_valid & row3_valid;
    ...
    always @(posedge i_clk) begin
    	if(i_rst) o_data <= 'd0;
    		...
    		o_data <= {row1[output_counter+2],row1[output_counter+1],row1[output_counter],
    				  row2[output_counter+2],row2[output_counter+1],row2[output_counter],
    				  row3[output_counter+2],row3[output_counter+1],row3[output_counter]};
    	end
    	else
    	   o_data <= 'd0;
    end
